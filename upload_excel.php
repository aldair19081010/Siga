...
try {
    ...
    for ($i = 1; $i < count($rows); $i++) {
        ...
        $nivel = $conn->real_escape_string(trim($rowData['nivel']));
        $grado = $conn->real_escape_string(trim($rowData['grado']));
        $seccion = $conn->real_escape_string(trim($rowData['seccion']));

        // Verificar si el estudiante ya existe en este colegio
        $existing_query = "SELECT id FROM student WHERE id_no = '$dni' AND school_id = $school_id";
        ...
        if ($existing->num_rows > 0) {
            ...
            $studentId = $existing->fetch_assoc()['id'];
            $update_query = "UPDATE student SET 
                ...
                grado = '$grado',
                seccion = '$seccion'
                WHERE id = $studentId AND school_id = $school_id";
            
            if (!$conn->query($update_query)) {
                throw new Exception("Error al actualizar estudiante: " . $conn->error);
            }
            
            $updated++;
            error_log("Estudiante actualizado: $dni - $name");
        } else {
            // Insertar nuevo estudiante con el school_id del administrador
            $insert_query = "INSERT INTO student (id_no, name, email, contact, address, nivel, grado, seccion, school_id) 
                           VALUES ('$dni', '$name', '$email', '$contact', '$address', '$nivel', '$grado', '$seccion', $school_id)";
            
            if (!$conn->query($insert_query)) {
                throw new Exception("Error al insertar estudiante: " . $conn->error);
            }
            
            $inserted++;
            error_log("Estudiante insertado: $dni - $name en colegio ID: $school_id");
        }
    }
    ...
    if ($conn->commit()) {
        ...
        echo json_encode(['status' => 'success', 'message' => "$inserted estudiantes agregados, $updated actualizados."]);
    } else {
