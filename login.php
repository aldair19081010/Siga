			...
			<div class="card-body">
				...
				<form id="login-form">
					<div class="form-group">
						<label for="school_id" class="control-label">Colegio</label>
...
<script>
	$('#login-form').submit(function(e) {
		...
		$.ajax({
			...
			dataType: 'json',
			error: function(xhr) {
				console.error("Error en la solicitud AJAX:", xhr.responseText || xhr);
				$('#login-form button[type="submit"]').removeAttr('disabled').html('Ingresar');
				$('#login-form').prepend('<div class="alert alert-danger">Error en el servidor. Intente nuevamente más tarde.</div>');
			},
			success: function(resp) {
				if (resp && resp.status == 1) {
					window.location.href = resp.redirect ? resp.redirect : 'index.php?page=home';
				} else if(resp && resp.message) {
					$('#login-form').prepend('<div class="alert alert-danger">' + resp.message + '</div>');
					$('#login-form button[type="submit"]').removeAttr('disabled').html('Ingresar');
				} else {
					$('#login-form').prepend('<div class="alert alert-danger">Respuesta inesperada del servidor.</div>');
					$('#login-form button[type="submit"]').removeAttr('disabled').html('Ingresar');
				}
			}
		});
